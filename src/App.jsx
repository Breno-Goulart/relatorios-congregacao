...
import { formatHoras, validarApenasLetras } from './utils/formatters';
...
const savedImage = localStorage.getItem('@Capa_App');
if (savedImage && typeof savedImage === 'string' && savedImage.startsWith('http')) {
...
export default function App() {
  ...
  today.setMonth(today.getMonth() - 1); // Volta um mês
  
  const defaultYear = today.getFullYear().toString();
  const defaultMonth = meses[today.getMonth()]; // Pega o nome do mês anterior na lista
  ...
  const publicadoresList = useMemo(() => {
  const publicadoresMap = new Map();
  
  if (!reports || reports.length === 0) return [];

  reports.forEach(report => {
    if (!report.nome) return;

    // Padroniza para Maiúsculas
    const nomeOriginal = report.nome.toUpperCase();
    
    // REGRA: Se não houver nome_busca no Firebase, criamos um aqui para o filtro funcionar
    const busca = report.nome_busca || nomeOriginal.normalize("NFD").replace(/[\u0300-\u036f]/g, "");

    // O Map evita nomes duplicados na lista de sugestões
    if (!publicadoresMap.has(busca)) {
      publicadoresMap.set(busca, { 
        nome: nomeOriginal, 
        nome_busca: busca 
      });
    }
  ...
  return Array.from(publicadoresMap.values()).sort((a, b) => a.nome.localeCompare(b.nome));
}, [reports]);
  
  useEffect(() => {
    if (currentUser || !db) return; // Only run for public users

    const fetchPublicNames = async () => {
      try {
        const querySnapshot = await getDocs(collection(db, 'relatorios'));
        const nameData = querySnapshot.docs.map(doc => ({
          nome: doc.data().nome,
          nome_busca: doc.data().nome_busca
        }));
        setReports(nameData);
      } catch (error) {
        console.warn("Aviso: A funcionalidade de autocomplete de nomes pode estar inativa devido às regras de segurança do Firestore. Erro: ", error.message);
      }
    };
  ...
  const saveConfig = useCallback(async (dadosRecebidos) => {
    try {
      ...
      if (dadosRecebidos instanceof File) {
        ...
        let compressedFile = dadosRecebidos;
        try {
          const options = {
            maxSizeMB: 0.3, // Limita a no máximo 300KB
            maxWidthOrHeight: 1200, // Redimensiona se for maior que 1200px
            useWebWorker: true // Usa multi-thread para não travar a tela
          };
          compressedFile = await imageCompression(dadosRecebidos, options);
          console.log(`Original: ${(dadosRecebidos.size / 1024).toFixed(2)} KB | Comprimida: ${(compressedFile.size / 1024).toFixed(2)} KB`);
        } catch (error) {
        ...
        const fileRef = ref(storage, `capas/capa_${Date.now()}_${compressedFile.name}`);
        
        // --- ADICIONANDO CACHE-CONTROL ---
        // Força o celular da pessoa a guardar a foto em cache por 1 ano
        const metadata = {
